...
- **NEVER merge 進正式主線**——就算違規 merge 了，雙重隔離仍能保證它不會被
  ...
  人工清理的污染
- **NEVER 把正式主線 merge 進 `test/*`**——守門基準取拓樸上最新的一顆 `test-sync:`
  或 `upstream-sync:`，merge 進來的正式主線 commit 跟 `test/*` 自己疊的快照互不為
  祖先，取到哪一顆不確定，merge 帶進的內容會被判成清單外改動而擋下；主線有更新
  （含清單改動）就刪掉 `test/*` 重切一條，不要 merge
