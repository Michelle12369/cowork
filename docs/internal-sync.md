...
同一份程式碼同時活在 GitHub（家裡，權威）與公司內網。兩邊的連通是**單向**的：
GitHub 經公司內 GitLab 鏡像流入，公司的 commit 永遠推不回 GitHub。策略不是「把
...
「上游（upstream）」指 GitHub 那份，經 GitLab 鏡像被公司消費——唯讀、不可修改、
...
`scripts/internal-owned-paths.txt` 列出的公司獨佔路徑在剛建立時，`develop` 上還
不存在——同步腳本靠 `git checkout develop -- <path>` 還原這些檔案，若它們從未被
commit 過，這一步會直接失敗。因此**首次同步 MUST 先由公司把各獨佔檔案 commit 到
`develop`**，之後才可能有東西可還原。
...
上游（含上游的刪除），再用獨佔路徑清單把公司檔案撈回來，最後在一條新切出的
...
- 同步 PR **MUST NOT squash 合併**：squash 會丟掉 commit 上的 `Upstream-Commit:`
  trailer，下一次同步就找不到基準點。這條規則 MUST 寫進公司側的 PR 流程說明。
...
| 類別 | 誰能寫 | 同步時 | 例子 |
|---|---|---|---|
| **共用權威檔** | 只有家裡 | 整檔取代 | `application.yml`、`pyproject.toml`、`uv.lock`、`requirements.txt`、`main.tsx`、`app/agent/**`（`runtime/internal_runtime.py` 除外）、`.env.example` |
...
- **`scripts/internal-owned-paths.txt`**——公司獨佔路徑，同步時先被上游整棵樹
  蓋掉、再從 `develop` 撈回來；也是守門檢查「獨佔清單外有沒有公司改動」的排除
  範圍。**新增公司獨佔檔時 MUST 同步更新這份清單**，否則下次同步會把它當成
  「越界改動」擋下，或者更糟——清單沒列到但也沒被上游覆蓋的檔案不會出現在守門
  裡，但一旦上游剛好新增同名路徑，該檔案會被無聲蓋掉。
- **`scripts/manual-merge-paths.txt`**——雙邊擁有檔，內容 MUST 是上面清單的
  子集：先被還原保住公司版，再由上游變更偵測攔下需要人工調和的情況（同步時若
  上游也動過該路徑，commit body 會多一行「需人工調和：<path>」）。

`uv.lock` 不在清單內——公司走 `requirements.txt`，不讀 lock；`requirements.txt`
...
requirements.txt`）而讓公司裝到舊依賴。`.env` 也不在清單內，它兩側皆 gitignored、
不在 index，`read-tree` 不會碰它。

家裡側的配合：這些獨佔路徑 **NEVER 加進家裡的 `.gitignore`**。公司必須能把獨佔檔
commit 到 `develop`，同步時 `git checkout develop -- <path>` 才有東西可還原；若
家裡把這些路徑寫進 `.gitignore`，該 `.gitignore` 會隨同步傳進公司，使這些檔案變成
被忽略，公司端得靠 `git add -f` 才追蹤得到，是一個沒必要的陷阱。

---

## 6. 守門的限制

守門的觀察範圍僅限 `develop`：若公司的越界改動還躺在未合併的 feature branch 上，
本次同步看不到，要等它 merge 進 `develop` 之後才會在**下一次**同步被攔下。這不是
漏洞（最終仍會被抓到），但延遲是真的——攔下的時間點可能離犯錯的當下很遠。

因此公司側的 code review MUST 一併把關「共用檔不得修改」，不能只依賴同步時的
守門作為唯一防線。
...
- `scripts/sync-upstream.sh` — 同步腳本本體（公司側執行、家裡維護）
