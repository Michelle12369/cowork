...
| Java proxy | emits `AUTH` for the ownership 404 and `INVALID_CALL` for a connector not in the session's `selectedConnectors` (with the allowed ids in `message`), before calling deepagent; passes every deepagent result through unchanged |
...
| `test_tool_call_empty_tool_name_returns_invalid_call` / `..._non_object_args_...` | row 3 |
