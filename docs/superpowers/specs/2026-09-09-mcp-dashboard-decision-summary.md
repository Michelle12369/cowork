...
| D10 (2) deepagent 側 | `REPAIR_SYSTEM_PROMPT_CONNECTOR`（[PR #87 comment 09-16](https://github.com/Michelle12369/cowork/pull/87#issuecomment-5695835479) 第 2 點: 明講錯誤來自瀏覽器而非 `check_dashboard`, 兩者檢查內容不同; `INVALID_CALL`／`TOOL_ERROR` 歸模型修, 其餘不動; raw 形狀）; `RepairRequest.connectors` 選填; 模式判定為 HTML 帶 prelude 或請求帶 connectors | [`app/agent/prompts.py`](https://github.com/Michelle12369/cowork/blob/feat/mcp-dashboard/deepagent-service/app/agent/prompts.py), [`repair_flow.py`](https://github.com/Michelle12369/cowork/blob/feat/mcp-dashboard/deepagent-service/app/agent/repair_flow.py), [`api/schemas.py`](https://github.com/Michelle12369/cowork/blob/feat/mcp-dashboard/deepagent-service/app/api/schemas.py) | [PR #87](https://github.com/Michelle12369/cowork/pull/87)（`0bb9319`, 開中） | `test_repair.py`（三條: file 模式原 prompt、connector 模式無清單、有清單）, `test_prompts.py` |
| U13 | `check.py` 移除鍵值抽取 helper 與「call-record checks not enabled」註記; tool docstring 與 SKILL.md 第 6 步改明講不比對 keys 與讀層 | [`app/agent/tools/check.py`](https://github.com/Michelle12369/cowork/blob/feat/mcp-dashboard/deepagent-service/app/agent/tools/check.py) | [PR #87](https://github.com/Michelle12369/cowork/pull/87)（`a68abcb`, 開中） | `test_check_dashboard.py` |
...
| D10 | 檢視期 `mcp()` 錯誤回到模型: (1) 前端 prelude 把 `{error}` 結果也發到既有 `erd-artifact-error` 通道（訊息含 arg keys 不含值）; (2) connector 版 `REPAIR_SYSTEM_PROMPT`（帶 connector 清單、`mcp()` 契約摘要、raw 形狀那句）, `RepairRequest.connectors`, Java `AnalysisBrowserRepairClient` 補帶 | **(1) 已落地**（[PR #83](https://github.com/Michelle12369/cowork/pull/83), 見 §2b; 訊息尚未帶 connector／tool／arg keys）. **(2) deepagent 側已落地**（`0bb9319`, 見 §2b; [主 spec §13](https://github.com/Michelle12369/cowork/blob/feat/mcp-dashboard/docs/superpowers/specs/2026-09-08-mcp-dashboard-on-autoland-design.md#13-決策紀錄) 09-16 條）. **未做**: Java `AnalysisBrowserRepairClient` 補帶 `connectors`（`ArtifactRepairService` 從 `ownedSession.getSelectedConnectors()` 經 `connectorCatalogService.resolveSpecs` 取, 與 `AgentOrchestrator` 同一呼叫）; 沒補帶時 prompt 只能沿用 HTML 既有的 connector 與 tool | Java 側隨 [PR #88](https://github.com/Michelle12369/cowork/pull/88) 之後的 Java 改動出 |
...
4. **`/repair` 的 connector prompt 沒有 connector 清單（09-16 更新, 缺口縮小）.** prompt 已知道 `mcp()`、raw 形狀與五個 code; 但 Java 還沒補帶 `connectors`, 所以 prompt 只能叫模型沿用 HTML 既有的 connector 與 tool, 不能糾正寫錯的 connector id.
