...
- **upstream（家裡）**：GitHub `Michelle12369/cowork`，dev 用 OpenRouter，deepagents + `ChatOpenAI`
- **internal 環境**：走內部 registry 與內部 lib

兩邊的連通是**單向**的：GitHub 經 internal GitLab 鏡像流入，internal 側的 commit **推不回 GitHub**。

已知會被 internal 側編輯的有 `pom.xml`、`application.properties`、`.env.example`、`index.html`、
...
但**internal 側的 commit 永遠回不到家裡**，於是：

- 共用檔上任何 internal 改動都是**孤兒**：家裡看不到，下一版照樣改同一行
...
單向也讓這條規則有了機械保障：internal 側**在物理上無法**把改動送回共用檔的權威來源。

---

## 核心原則：家裡開接縫，internal 填實作

**家裡是唯一的權威寫入者。** internal 側對 fetch 下來的共用檔一行都不改；任何「非改不可」的
...
| **internal 獨佔檔** | 只有 internal | **取代後還原** | `src/internal/java/**`、`internal.impl.ts`、`internal_runtime.py`、`internal/requirements-internal.txt` |
...
牆的另一側有 1289 行 import langchain/langgraph/deepagents，而 internal 要換掉的
`create_deep_agent`、model、checkpointer 正是這 1289 行的支柱。

**在 internal 改動永遠回不了家的前提下維護 1289 行雙份程式碼，必然發散到無法互相套用**——
每次上游同步，那 1289 行都要重新人工調和一次。這是整個問題裡唯一昂貴的部分。

### 為什麼可以做得很薄

internal lib 是**包在 langgraph/langchain 之上的 wrapper**，底層仍是 langgraph。因此兩個實作
...
# 三個建構點的 factory。型別一律用 langchain/langgraph base type：internal wrapper 底層同為
...
| `InternalRuntime` | `app/agent/runtime/internal_runtime.py` | **internal 獨佔**（家裡不存在；internal 側 commit 於 `develop`，同步時還原） |

以 `AGENT_RUNTIME=deepagents|internal` 選擇（預設 `deepagents`）。

**設為 `internal` 但實作檔不存在時 MUST 啟動即失敗並指出缺少的檔案，NEVER fallback 回
deepagents。** 靜默降級會讓 internal 環境跑在錯誤的 runtime 上而無人察覺，比啟動失敗糟得多。
...
- `auth.py`（156 行 token-exchange http client）只被家裡的 `build_model()` 呼叫。internal
  runtime 自行處理 auth，該檔在 internal 環境不會被執行——**不需要分叉，也不需要刪**。
...
### 前提：internal 環境仍安裝 deepagents

`graph.py` 保留在共用權威檔，而它 module 層 import 了 `deepagents`
（`FilesystemBackend`、`register_harness_profile`）。這在 internal 環境成立的前提是
**internal registry 有 deepagents**——`pyproject.toml` 的既有註解「internal registry 僅有 0.5.x；
開發與生產一律鎖同一版」已確認此點。

若此前提日後不成立（internal 完全移除 deepagents），`DashboardOverwriteBackend` 與
`register_harness_profile` 必須一併移入 runtime 實作，接縫會從三個方法擴大到五個。
**這是本設計對外部條件最脆弱的一處，變更前 MUST 重新評估。**

### 已確認相容

- **backend 參數**：internal wrapper 接受 deepagents 的 `FilesystemBackend` 子類，
  `DashboardOverwriteBackend` 可原樣傳入 → single-write 不變量
  （`dashboard.html` 只能整份重寫）在 internal 環境維持不變。
...
- **tools 的定義形式**。若 internal wrapper 不吃標準 `@tool` 物件，需要第四個 factory
...
前三個接縫（Python runtime、Vite 注入、bootstrap glob）都能做到 internal 零編輯且家裡零成本。
`pom.xml` 不行——要維持單一份 pom，代價是 `internal` profile ＋ GAV property 佔位符
＋ build-helper ＋ internal 端 `settings.xml` ＋ 家裡的替身驗證，五層間接只為了讓 internal 多一個
`<dependency>`。

**接縫成本超過分歧成本時，正確答案是接受分歧並偵測它，而不是硬造接縫。**
`pom.xml` 因此改列**雙邊擁有檔**：internal 持有自己的版本，上游一有變更就停下來人工調和。

### internal 側的 `pom.xml`

＝上游版本 ＋ 兩塊：

1. internal SDK 的 `<dependency>`（座標直接寫，因為這份 pom 不會回到 GitHub）
...
改列**雙邊擁有檔**：internal 側直接在自己那份裡設定 `erd.upload.decryption.enabled=true`、
`tsso.enabled=true` 等鍵，同步時還原 internal 版本，上游若也動過該檔則走人工調和（同
...
依賴變更（新增 commons-io、移除 S3 路線）。每次上游動到它，internal 都要人工套一次。

這個代價可接受的前提是**偵測必須可靠**：漏掉一次上游變更，internal 就會缺依賴或帶著已移除
的依賴繼續跑，而症狀可能要很久才浮現。偵測機制見〈搬運流程〉的 `$LAST_UPSTREAM` 錨點；
實際調和動作在同步 branch 上完成，隨該次 PR 一起進 `develop`。

## 接縫三：frontend

### `index.html` — 雙邊擁有檔，internal 直接編輯
...
改採的做法：internal lib 仍是 index.html 掛的 global script（非 npm 套件），因此
`package.json` 與 lock 檔完全不受影響；但改由**internal 側直接編輯 `frontend/index.html`**，
在 `<head>` 依相依順序加三支 classic script（NEVER `type="module"`、NEVER
`async`/`defer`，才能保證在 `main.tsx` 之前同步執行完畢）。`frontend/index.html`
因此同時列進兩份清單：

- `scripts/internal-owned-paths.txt`——同步時先被上游整棵樹蓋掉，再從 internal 的
  `develop` 撈回來，internal 的 script 標籤不會被同步抹掉。
- `scripts/manual-merge-paths.txt`——上游若也動過 `index.html`，同步 commit 的
  body 會多一行「需人工調和」提示，人工比對兩邊版本後決定怎麼合併。

兩份清單都要加：只加前者，上游改了 `index.html` 沒人會知道；只加後者，internal 的
script 標籤每次同步都會被抹掉。
...
`main.tsx` 在 `createRoot` 之前 `await initInternalRuntime()`。internal 只需放進
`internal.impl.ts`（家裡不存在；internal 側 commit 於 `develop`，同步時還原），並確保
`index.html` 已掛好三支 script（見上節）；`initialize` 怎麼呼叫、傳什麼參數，家裡不需要知道。

### `internal.impl.ts` 範例（internal 側撰寫）

最可能的用途就是 CLAUDE.md 已載明的那件事——**internal 環境的 `X-User-Id` 改由 SSO 提供**，
取代 v1 的 localStorage 匿名 UUID：

```ts
// frontend/src/bootstrap/internal.impl.ts
// internal 環境專屬；家裡不存在此檔，由 internal.ts 的 import.meta.glob 偵測後載入。
// internal library 是 index.html 掛的 global script，這裡不再自己載入，只做 guard + init。
...
internal 實作若直接 `localStorage.setItem('erd_user_id', ...)` 硬寫同一個 key，
可以完全不改主線——但那是**隱性耦合**：哪天主線改了 key 名稱，internal 端不會編譯錯誤、
...
三行，且對家裡完全無副作用（沒有人呼叫它）。**這是本設計唯一要求主線為 internal 開的洞**——
...
家裡 v1 無 SSO，身分靠 `X-User-Id` header ＋ `local-dev` fallback。internal 環境改由 TSSO 提供，
且多一個 `deptId` 維度。
...
| `true`（internal） | **不註冊**主線 interceptor；身分由 internal TSSO 提供，internal 在 `src/internal/java` 放自己的 `WebMvcConfigurer` |
...
Spring 允許多個 `WebMvcConfigurer` 並存，故 internal 側新增自己的 config 不需要主線配合。

**`tsso.enabled=true` 但 internal 尚未提供 config 時**，沒有任何 interceptor 會填 `CurrentUser`。
...
internal 環境的問題家裡重現不了，log 是唯一的線索。**接縫的分支點 MUST 留下 log**——
...
| `requirements.txt` | **internal 環境** | `uv export --no-dev --no-hashes --format requirements-txt` |

因此 `uv.lock` **不進 internal 獨佔路徑清單**——internal 不讀它，同步時被上游整檔取代是正確行為。
現行 export 命令的 `--no-hashes` 也已經對內部 mirror 友善：mirror 的 artifact 與公有
PyPI 的 hash 不同，帶 hash 會直接安裝失敗。

internal 專屬套件（內部 wrapper lib）NEVER 進共用 `pyproject.toml`——它在家裡解析不到，
會讓家裡的 `uv lock` 直接失敗。改放 internal 獨佔的 `internal/requirements-internal.txt`，
於共用 `requirements.txt` 之後安裝。

`pyproject.toml` 現有的 `deepagents==0.5.5`（註解已載明「internal registry 僅有 0.5.x」）
維持不變：關鍵套件 MUST pin 到 internal registry 供得起的版本，這是共用檔的既有紀律。

### ⚠️ `requirements.txt` 是生成物——漂移只會在 internal 炸

`requirements.txt` 由 `uv.lock` 匯出。若改了依賴卻忘記重新 export，家裡完全無感
（家裡走 `uv sync --frozen`，讀的是 lock），**internal 卻會靜默裝到舊依賴**。

這正是〈已知代價〉所說「家裡永遠測不到 internal 路徑」的具體案例，而且是最容易發生的一種——
...
不一致即失敗。**這是少數能在家裡就攔下 internal 環境問題的機制**，價值遠高於它的實作成本。
...
若 internal 側的越界改動還躺在未合併的 feature branch 上，本次同步看不到；
等它們 merge 進 `develop`，會在**下一次**同步才被攔下。

這不是漏洞（最終仍會被抓到），但**延遲是真的**——攔下的時間點可能離犯錯的人很遠。
internal 側的 code review MUST 一併把關「共用檔不得修改」，不要只依賴同步時的守門。

### 用語

「上游（upstream）」＝ GitHub 那份，經 GitLab 鏡像被 internal 消費——**唯讀、不可修改、
...
且同步 commit 訊息記了是哪一版，事後查得到。但**家裡剛 push 就要 internal 同步時，
...
〈硬約束〉，internal 那側的改動永遠回不了家，同一個衝突會無限重複。改用
**取代後還原**——`git read-tree -u --reset` 把整個 worktree 換成上游（含上游的刪除，
這是 `git checkout` 做不到的），再把 internal 獨佔路徑撈回來。**因此永遠不會有 conflict**，
...
### 腳本（`scripts/sync-upstream.sh`，internal 側維護）
...
# （上次同步的上游 commit），NEVER 用 $LAST_SYNC——後者的 pom.xml 已是 internal 版，
...
git checkout develop -- "${OWNED[@]}"             # 還原 internal 獨佔路徑（淨變更為零）
git add -A
...
### internal 獨佔路徑清單（`scripts/internal-owned-paths.txt`，單一事實來源）
...
它的內容 MUST 是上面清單的子集：先被還原保住 internal 版，再由上游變更偵測攔下需人工調和的情況。

`uv.lock` **不在清單內**——internal 走 `requirements.txt`，不讀 lock（見上節）。
`.env` 也不在，它 gitignored、不在 index，`read-tree` 不會碰它。

清單首次建立時這些路徑在 `develop` 上還不存在，`git checkout develop -- <path>` 會失敗；
**首次同步 MUST 在 internal 先 commit 各獨佔檔之後才跑。**

### 守門檢查是這個流程唯一的安全裝置

`read-tree --reset` 會**無聲抹掉**獨佔清單以外的一切 internal 改動；`git add -A` 則會把 internal
...
internal 必須把獨佔檔 commit 到 `develop`，`git checkout develop -- <path>` 才有東西可還原。
若家裡把這些路徑寫進 `.gitignore`，該 `.gitignore` 會隨 ③ 傳進 internal，使那些檔案變成
ignored，internal 得靠 `git add -f` 才能追蹤——一個沒必要的陷阱。
...
| 測試 | 內容 |
|---|---|
| `AgentRuntime` 契約測試 | 對 runtime 實作跑同一份測試（家裡跑 `DeepAgentsRuntime`，internal 跑 `InternalRuntime`）：三個 build 方法回傳的物件滿足對應 base type、`build_agent` 產出的 graph 可被 `astream_events` 驅動 |
...
| 同步腳本守門測試 | 在拋棄式 repo 上驗證**皆中止**：① 獨佔清單外有 internal 改動 ② 有野生 untracked 檔 ③ 不在 `develop` 上 ④ 找不到基準同步 commit（首次同步未 bootstrap）。守門是整個流程唯一的安全裝置，MUST 有自動化驗證 |
...
家裡**永遠測不到 internal 路徑**——所有接縫在家裡都是 no-op 或 deepagents 實作。對策有二：
...
- internal lib 的實際實作（由 internal 環境提供）
- internal tool 定義形式的適配（資訊不足，待 internal lib 文件到手後另議）
- internal 側 `settings.xml`、內部 registry、proxy、CA 憑證的設定內容（屬環境建置，非本專案程式碼）
