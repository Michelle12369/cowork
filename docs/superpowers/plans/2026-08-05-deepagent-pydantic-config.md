...
> **後續修訂（已執行，不影響本檔歷史紀錄——執行完成的 plan 是 point-in-time 記錄，內文原樣
> 保留）：** 本檔案下方的 Langfuse runtime seam（`build_langfuse_mask`，只給 mask function）
> 後續已擴大為 `build_langfuse`（完整接管 client 建構）。現行版本見
> `docs/superpowers/specs/2026-08-05-deepagent-pydantic-config-design.md` §2 與
> `docs/internal-implementation-guide.md`「Langfuse seam」小節。
