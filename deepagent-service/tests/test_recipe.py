...
from app.engine.api_fetch import record_fetch
...
def test_build_recipe_htmlReferencesOnlyQ1_queriesContainsOnlyQ1(tmp_path):
    workspace = _make_workspace(tmp_path)
    record_fetch(workspace, "yield_data", "mes_yield", {"line_id": "A"}, ["line_id", "yield"])
...
def test_build_recipe_lastWinsPerAlias(tmp_path):
    workspace = _make_workspace(tmp_path)
    record_fetch(workspace, "yield_data", "mes_yield", {"line_id": "A"}, ["line_id"])
    record_fetch(workspace, "yield_data", "mes_yield", {"line_id": "B"}, ["line_id"])
