...
from app.agent import chat_turn, critic
from app.agent.critic import CriticVerdict
...
# -- final critic (ERD_FINAL_CRITIC) --------------------------------------------------------
#
# 預設由 conftest 的 `_isolate_final_critic` autouse fixture 關閉；這裡的測試各自
# monkeypatch.setenv("ERD_FINAL_CRITIC", "true") 開啟，並 monkeypatch `critic.run_final_critic`
# 換上腳本化的判定結果，不依賴真的模型呼叫。


def _scripted_critic_verdicts(verdicts: list[CriticVerdict | None]):
    """回傳一個可以當 `critic.run_final_critic` 用的 async 假函式，依序 pop `verdicts`；
    腳本耗盡時回 None（fail-open），跟 ScriptedChatModel 回放耗盡時的容錯精神一致。"""
    ...
    return _fake_run_final_critic


@pytest.fixture()
def scripted_flow_critic_claims_without_writing(tmp_path, monkeypatch):
    """重現生產 Langfuse trace 的假成功案例(「200跟300看起來一模一樣」那輪):模型只呼叫
    run_sql、從未 write_file dashboard.html,卻用文字聲稱已經改好。腳本只到這裡為止——不含
    任何修正輪內容,用來測修正輪本身也交不出東西時的收尾。"""
    monkeypatch.setenv("AGENT_WORKSPACE_ROOT", str(tmp_path / "ws"))
    scripted = ScriptedChatModel(
        [
            AIMessage(
            ...
            AIMessage(content="已將 dashboard 更新,您可直接檢視。"),
        ]
    )
    monkeypatch.setattr(chat_turn, "build_model", lambda: scripted)
    return scripted
...
@pytest.fixture()
def scripted_flow_critic_corrective_writes(tmp_path, monkeypatch):
    """同一個假成功案例,但腳本延續到修正輪:修正輪真的補上 write_file。修正輪的第一步
    MUST 先讀 skill(不同於既有的 guard-repair-loop 先例)——原始輪從未寫過 dashboard.html,
    這個 thread 至此還沒讀過 skill,DashboardSkillGateMiddleware 會擋。"""
    monkeypatch.setenv("AGENT_WORKSPACE_ROOT", str(tmp_path / "ws"))
    scripted = ScriptedChatModel(
        [
            AIMessage(
            ...
            AIMessage(content="已將 dashboard 實際更新完成,請查看右側預覽。"),
        ]
    )
    monkeypatch.setattr(chat_turn, "build_model", lambda: scripted)
    return scripted
...
async def test_chat_final_critic_ok_matches_normal_flow(
    tmp_path, scripted_flow, monkeypatch
) -> None:
    ...
    monkeypatch.setattr(
        critic,
        "run_final_critic",
        _scripted_critic_verdicts([CriticVerdict(ok=True, issues=[], fix_instruction="")]),
    )
    events = await _post_chat(tmp_path)

    step_keys = [event.get("stepKey") for event in events if event["type"] == "STEP"]
    assert "final_critic" not in step_keys
    dashboard_events = [event for event in events if event["type"] == "DASHBOARD_HTML"]
    assert len(dashboard_events) == 1
    assert events[-1] == {"type": "ANSWER", "text": "CRM 系統工單最多,最需要改善。"}


async def test_chat_final_critic_corrective_round_writes_dashboard_no_prefix(
    tmp_path, scripted_flow_critic_corrective_writes, monkeypatch
) -> None:
    monkeypatch.setenv("ERD_FINAL_CRITIC", "true")
    monkeypatch.setattr(
        critic,
        "run_final_critic",
        _scripted_critic_verdicts(
            [
                CriticVerdict(
                    ok=False,
                    issues=["draft claims a dashboard update but no write_file happened this turn"],
                    fix_instruction="Rewrite dashboard.html via a single write_file call.",
                ),
                CriticVerdict(ok=True, issues=[], fix_instruction=""),
            ]
        ),
    )
    events = await _post_chat(tmp_path)
    ...
    step_keys = [event.get("stepKey") for event in events if event["type"] == "STEP"]
    assert "final_critic" in step_keys
    ...
    answer_events = [event for event in events if event["type"] == "ANSWER"]
    assert len(answer_events) == 1
    assert not answer_events[0]["text"].startswith(chat_turn.CRITIC_INCONSISTENT_PREFIX)
    assert answer_events[0]["text"] == "已將 dashboard 實際更新完成,請查看右側預覽。"


async def test_chat_final_critic_still_inconsistent_after_corrective_round_gets_prefix(
    tmp_path, scripted_flow_critic_claims_without_writing, monkeypatch
) -> None:
    monkeypatch.setenv("ERD_FINAL_CRITIC", "true")
    monkeypatch.setattr(
        critic,
        ...
        _scripted_critic_verdicts(
            [
                ...
                CriticVerdict(
                    ok=False,
                    issues=["still no write_file after the corrective round"],
                    fix_instruction="",
                ),
            ]
        ),
    ...
    answer_events = [event for event in events if event["type"] == "ANSWER"]
    assert len(answer_events) == 1
    assert answer_events[0]["text"].startswith(chat_turn.CRITIC_INCONSISTENT_PREFIX)
    assert "已將 dashboard 更新,您可直接檢視。" in answer_events[0]["text"]


async def test_chat_final_critic_none_verdict_fails_open(
    tmp_path, scripted_flow, monkeypatch
) -> None:
    monkeypatch.setenv("ERD_FINAL_CRITIC", "true")
