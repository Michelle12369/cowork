...
def test_table_event_keyed_by_run_id_not_shared_across_bridges() -> None:
    ...
    table_one = next(event for event in emitted_one if event["type"] == "TABLE")
    table_two = next(event for event in emitted_two if event["type"] == "TABLE")
