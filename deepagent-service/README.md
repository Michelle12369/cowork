...
服務不支援 `.env` 檔：設定來源是 env var 與 `one.properties`／`one-local.properties` 這一份
properties 檔，加上欄位預設，優先序 env > 檔案 > 預設；沒有 dotenv 解析這一層。兩層都是真的
部署路徑——compose 沒掛 properties 檔，整包設定都走 env；internal 掛實值版的 properties
檔。若設定是放在環境變數裡，
