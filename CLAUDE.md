...
- **Artifact 契約**：所有 provider 產出的 HTML 從 `window.__ERD_DATA__[alias]` 讀資料；後端 ArtifactAssembler 注入全量資料（抽樣機制已移除——目前資料量級不需要；LLM 只看 schema/統計摘要/樣本列不變）；統計計算在瀏覽器 JS。迭代修改時回餵前版 raw HTML + 最小變更指令；進度顯示由模型 `[[step:]]` 標記驅動。**analysis 模式（`langgraph-analysis`，經 `deepagent-service`）另有兩種契約**：檔案模式——數字由 DuckDB SQL 算出，只把被引用的查詢結果注入 `window.__ERD_RESULTS__["qN"]`，瀏覽器只做渲染、NEVER 在頁面 JS 算統計；connector 模式——頁面在檢視時透過宿主提供的 `mcp(connector, tool, args, handler)` 現抓資料，不注入資料，數字由頁面 JS 從 raw 回應算出（契約與決策見 `docs/superpowers/specs/2026-09-08-mcp-dashboard-on-autoland-design.md` 與 `2026-09-09-mcp-dashboard-decision-summary.md`）。**品質責任**：dashboard 建立者在 publish 前負責正確性；系統提供驗證資訊（語法／契約檢查、錯誤回報），不替建立者自動阻擋。
- **LLM providers**（可插拔，`erd.agent.provider`，統一回傳 AgentEvent 流：STEP/TOKEN/ANSWER/ARTIFACT/ERROR/QUESTION/THINKING）：OpenAICompatibleProvider（預設，OpenAI-compatible `/v1/chat/completions` SSE——internal LLM 或 OpenRouter；internal 環境支援 auth-mode=token-exchange（j1→j2，TTL 快取，401 自動重試））、LangGraphAnalysisProvider（`langgraph-analysis`：轉發到 `deepagent-service` :8000 的 `/chat` SSE 與 `/repair`；Python 端以 deepagents runtime＋DuckDB 工具（get_schema／preview_data／run_sql）＋connector 工具跑 data-analyst agent，dashboard 走 `write_file dashboard.html`＋skill gate＋`check_dashboard`）。InternalCodegenProvider 已移除（internal 環境改走 OpenAI-compatible 或 analysis 模式）。
...
- **狀態（2026-09-10，本 branch `feat/mcp-dashboard`）**：Phase A 已經 PR #81 merge 進本 branch（merge commit `919be87`）：connector 呼叫每輪自動落成只活一輪的 DuckDB 表（D5 raw 形狀回饋）、connector 模式 dashboard 一律 `mcp()` 現抓（D6）、`mcp-data-dashboard` skill＋skill gate（D7）、`check_dashboard` 最小形態（語法＋契約 lint，D1–D4 (i)）、spike 宿主 `spike/mcp-shell`（throwaway，唯一能開頁的宿主）。**未落地**：Checkpoint A 人工測試與 spike 快照（D8）、call record `connector_calls.jsonl`（Phase B）、D9 傳輸面——deepagent `/tool-call`＋prelude 已落地於 `feat/mcp-tool-call`（PR 待開進 `feat/mcp-dashboard`；錯誤碼五個，見 `2026-09-10-mcp-error-codes-design.md`），前端 bridge（hop ②）與 Java `/mcp-call`（hop ③）未落地、D10 檢視期錯誤回到模型。**D11 定為不做**（deepagent 內不模擬瀏覽器：QuickJS／jsdom／Chromium 皆不做），只在 Checkpoint A 或 Langfuse 證明缺口時重開（U8）。下一個 PR 先做 Phase B 或 D9+D10 由 Checkpoint A 觀察決定（U1）。過渡期已知缺口見決策總結 §5（沒有錯誤自動回模型、只有 spike 宿主、`/repair` 仍是 file 模式 prompt、`node` 不進 image）。**未來環境**：可能取得 agent 端 remote sandbox（遠端執行程式碼）與 VLM／更強模型——屆時一併重評 D11／U8、gpt-oss 時期的 rails（skill gate、微規則、確定性結構）與 harness 結論；相關腦力激盪（headless Chromium 宿主、QuickJS REPL＋PTC、三語言分工）尚未成 spec，NEVER 當已定案引用。
- **狀態（2026-07-29，歷史；master 與 PR #2 現況本 clone 無法驗證，以 GitHub 為準）**：master＝v1.2.0（`7081dae`）。分支 `feat/data-insight-agent`（HEAD `ed6b447`，**全數 opus 過審未 merge**，PR #2 open）：M1（LangGraph 分析 provider 接入）＋ M2/M2.5 T14–T24（節點拆分 gather/synthesize、ask_user 反問、記憶壓縮、注入防護、spec 持久化 V10/V11、synthesize 補救迴路、gather 首輪重試、renderer 全面打磨含 SPC/boxplot/單位/洞察卡/疊加圖）＋ T26 custom_chart 逃生艙（模型直寫 ECharts option JSON，佔位符綁定＋單卡 try/catch，安全性 opus 紅隊過）。dataviz skill 已蒸餾進 dashboard 模式 prompt＋erd theme 換 8 色 CVD 安全盤。三側測試 647/505/312 綠。**實驗分支 `exp/custom-chart-only`**（`d01033a`）：九種標準圖型 gate 掉、全走 custom——三臂對照實驗中（純 renderer=tag `m2.5-renderer-baseline`/混合=feat HEAD/全 custom），NEVER merge 回主線。**merge master 路徑已定**：整條 feat 進（不切子集，三側逐 task 共生），前置＝opus delta 終審 `6a98e79..ed6b447`（T24 終審之後的部分）→ PR #2 更新 → 使用者觸發 merge。模型：目前為 deepseek-v4-flash（2026-09 更新；先前 gpt-oss 時期採「確定性結構」品質策略，其 rails 是否放寬待評估，可申請 VLM／更強模型）；dev 經 OpenRouter。**待辦積壓**：數字落地核對（確定性，殺抄錯）、分析劇本 skills 化（load_analysis_skill 漸進揭露；M3 蒸餾手法的容器格式）、series_column pivot 量級護欄盲區、五題前沿模型對照實驗、deep-agent harness 評估結論＝gpt-oss 時期定為不換 harness 只採 skills 概念（模型已換，待重評）。**接續事項見 `.superpowers/sdd/progress.md` 尾端**。前端 :3000、backend :${BACKEND_PORT:-8080}。
