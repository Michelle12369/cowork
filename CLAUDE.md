...
- **檔案**：限 5 檔/session 共 5GB（CSV 單檔 2GB 串流解析、xlsx 上限 200MB）；儲存走 PVC RWX 單一路線（`FileStorage` 介面只剩 `LocalDiskStorage`，S3/MinIO 已移除）；分級保留（artifact HTML 2 年、workspace 與上傳原始檔依 session 最後活動 180 天，皆可用 `ERD_STORAGE_RETENTION_*` 調整，並有 `ERD_STORAGE_CLEANUP_DRY_RUN`）。
