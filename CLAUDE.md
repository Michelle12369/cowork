...
- **狀態（2026-09-10，本 branch `feat/mcp-dashboard`，PR #87 開進 `feat/9E`）**：Phase A 已經 PR #81 merge 進本 branch（merge commit `919be87`）：connector 呼叫每輪自動落成只活一輪的 DuckDB 表（D5 raw 形狀回饋）、connector 模式 dashboard 一律 `mcp()` 現抓（D6）、`mcp-data-dashboard` skill＋skill gate（D7）、`check_dashboard` 最小形態（語法＋契約 lint，D1–D4 (i)）、spike 宿主 `spike/mcp-shell`（throwaway，唯一能開頁的宿主）。**未落地**：Checkpoint A 人工測試與 spike 快照（D8）、call record `connector_calls.jsonl`（Phase B）、D9 傳輸面——deepagent `/tool-call`＋prelude 已經 PR #83 merge 進本 branch（錯誤碼五個，見 `2026-09-10-mcp-error-codes-design.md`），前端 bridge（hop ②）與 Java `/mcp-call`（hop ③）已於 `feat/mcp-dashboard-host`（2026-09-15，自本 branch 分出，PR #88 開進本 branch）落地，spec `2026-09-15-mcp-host-bridge-design.md`、D10 檢視期錯誤回到模型。**D11 定為不做**（deepagent 內不模擬瀏覽器：QuickJS／jsdom／Chromium 皆不做），只在 Checkpoint A 或 Langfuse 證明缺口時重開（U8）。下一個 PR 先做 Phase B 或 D9+D10 由 Checkpoint A 觀察決定（U1）。過渡期已知缺口見決策總結 §5（沒有錯誤自動回模型、只有 spike 宿主、`/repair` 仍是 file 模式 prompt、`node` 不進 image）。**未來環境**：可能取得 agent 端 remote sandbox（遠端執行程式碼）與 VLM／更強模型——屆時一併重評 D11／U8、gpt-oss 時期的 rails（skill gate、微規則、確定性結構）與 harness 結論；相關腦力激盪（headless Chromium 宿主、QuickJS REPL＋PTC、三語言分工）尚未成 spec，NEVER 當已定案引用。
