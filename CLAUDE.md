...
- **檔案**：限 5 檔/session 共 5GB（CSV 單檔 2GB 串流解析、xlsx 上限 200MB）；儲存雙路線（`erd.storage.type`／`STORAGE_BACKEND`＝`local|s3`；s3 走 write-once generation 快照，本機/compose 用 MinIO，internal 現行路線）；分級保留（artifact HTML 2 年、workspace 與上傳原始檔依 session 最後活動 180 天，皆可用 `ERD_STORAGE_RETENTION_*` 調整，並有 `ERD_STORAGE_CLEANUP_DRY_RUN`）。deepagent-service 另有 API connector 機制（`AGENT_CONNECTORS_FILE` 設定才啟用，未設＝功能全關）：`fetch_api_data` 抓回的 snapshot 落 workspace 隨 generation 快照跨 turn 重掛，dashboard 用 `data-bind` 屬性做敘事綁定（確定性 resolver 注入填值，不經 LLM 對數字）。
