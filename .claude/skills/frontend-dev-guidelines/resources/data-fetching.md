...
> **Important (TanStack Query v5):** `useSuspenseQuery` has **no `onError` option** — it was removed in v5. Errors thrown by the query propagate up and are caught by the nearest `<ErrorBoundary>`. This is the correct pattern: separate the "happy path" (component) from the "error path" (boundary). `onError` remains valid only on `useMutation`.

### Error Boundaries (the way to handle query errors)
...
// Query errors bubble up to here — no onError needed on the query itself
...
### Mutation errors (onError is valid here)
...
const mutation = useMutation({
    mutationFn: (payload) => myFeatureApi.update(id, payload),
    onError: (error) => {
        message.error('Failed to save changes');  // ✅ valid on mutations
...
5. **Route Format**: `/chat/route` NOT `/api/chat/route`
6. **Mutations**: invalidateQueries after success
7. **Error Handling**: `<ErrorBoundary>` for query errors; `onError` + antd `message` for mutation errors
