...
// For grouped Tailwind class strings
const classes: Record<string, string> = {
    container: 'flex flex-col p-4',
    header: 'mb-2 text-lg font-semibold',
};
```
...
```typescript
// ✅ Constrain the generic instead of casting to any
function getById<T extends { id: number }>(items: T[], id: number): T | undefined {
    return items.find(item => item.id === id);  // item.id is type-safe, no cast
}
